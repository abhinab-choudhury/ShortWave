      ...
      <div className="flex flex-col gap-3 sm:gap-3.5 min-w-0">
        ...
        <div className="flex flex-wrap items-center justify-between gap-2 text-sm text-muted-foreground border-t border-dashed border-slate-100 dark:border-slate-800 pt-3 mt-1">
          <div className="flex items-center gap-2 min-w-0 flex-1">
            ...
            <span className="truncate font-mono text-xs sm:text-[13px] text-slate-700 dark:text-slate-300 min-w-0" title={short_link}>
              {short_link}
            </span>
          </div>
          <span className="text-[11px] sm:text-xs text-slate-500 dark:text-slate-400 whitespace-nowrap shrink-0">{DateUtils.timeAgo(created_at)}</span>
        </div>
      </div>
    </div>
  )
