    ...
    >
      <div className="flex items-start sm:items-center justify-between gap-3 sm:gap-4 min-w-0">
        <div className="flex flex-col gap-1 sm:gap-1.5 min-w-0 flex-1">
          <p className="text-xs sm:text-sm font-medium tracking-wide text-teal-800 dark:text-teal-300 truncate">
            {title}
          </p>
        ...
        <div
          className={cn(
            icon_styles,
            "p-2.5 sm:p-3 rounded-full sm:rounded-2xl shrink-0 flex items-center justify-center",
          )}
        >
          {icon}
        </div>
      </div>
      <p className="mt-3 text-xs sm:text-sm leading-5 sm:leading-6 text-teal-700/70 dark:text-slate-400 line-clamp-1">
        {footer}
      </p>
    </div>
  );
