...
export function CopyShortUrlBtn({
  ...
  const handleCopy = async () => {
    ...
    try {
      await navigator.clipboard.writeText(
        `${import.meta.env.VITE_SERVER_URL}/${shortLink}`,
      );
      setCopied(true);
      onCopy?.(shortLink);
      toast({
  ...
  return (
    <Button
      ...
      size={size}
      className={cn(
        "p-2 cursor-pointer active:scale-95 active:bg-accent/40 transition",
        className,
      )}
    >
      {isCopying ? (
      ...
      <span className="text-sm font-medium leading-6 sr-only">
