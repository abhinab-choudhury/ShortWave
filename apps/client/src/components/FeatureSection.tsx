...
const TABS: {
...
}[] = [
  {
    ...
    cardDesc:
      "Unlock every tool without limits — no hidden fees, no subscriptions.",
    features: [
      {
        icon: (
  ...
  },
  {
    ...
    cardDesc:
      "Keep track of progress with automated reporting and status updates.",
    features: [
      {
        icon: (
    ...
    <section className="w-full border-y border-slate-200/60 dark:border-slate-800 bg-white dark:bg-slate-950">
      <div className="mx-auto max-w-6xl px-6 py-16 md:py-20">
        <div className="text-center space-y-4">
          <p className="text-xs uppercase tracking-[0.2em] font-medium text-teal-600 dark:text-teal-400">
            What sets this apart
          </p>
          <h2 className="text-3xl md:text-4xl lg:text-5xl font-extrabold tracking-tight text-balance text-slate-900 dark:text-white">
            Features
          </h2>
          <p className="mx-auto max-w-2xl text-sm md:text-base leading-relaxed text-slate-500 dark:text-slate-400">
            All the advantages that simplify link management and analytics —
        ...
        <Tabs defaultValue={TABS[0].key} className="mt-12 md:mt-16">
          ...
          {TABS.map(({ key, cardTitle, cardDesc, features }) => (
            <TabsContent
              key={key}
              value={key}
              className="mt-10 md:mt-12 focus-visible:outline-none"
            >
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 md:gap-10">
                <Card className="bg-gradient-to-br from-teal-50/50 to-white dark:from-slate-800/50 dark:to-slate-900/50 border-slate-200/60 dark:border-slate-800 shadow-sm">
                  <CardContent className="h-full p-6 md:p-8 flex flex-col justify-between">
                    ...
                    <div className="pt-6">
                      <Button className="bg-teal-600 hover:bg-teal-700 text-white font-semibold shadow-sm hover:shadow-md transition-all duration-200 hover:scale-[1.02] active:scale-[0.98] dark:bg-teal-500 dark:hover:bg-teal-400 dark:text-slate-950">
                        Get Started
                      </Button>
                    </div>
                  </CardContent>
...
export function FeatureItem({
  ...
  return (
    <div className="flex items-start gap-4 group">
      <div
        className="inline-flex p-2.5 items-center justify-center rounded-xl border border-teal-100 bg-teal-50 dark:border-teal-900/50 dark:bg-teal-950/40 shrink-0 transition-transform duration-200 group-hover:scale-110"
        aria-hidden="true"
      >
        {icon}
      </div>
